...
def get_data_transform():
    # base de dados
    lfw_allnames = pd.read_csv("unique.csv")
    ...
    image_paths['image_path'] = image_paths.name + "_" + image_paths.image_path + ".jpg"
...
def calfw_mount_data(classes_list):
    image_paths = get_data_transform()
    total_filtered = image_paths[image_paths['name'].isin(classes_list)]
...
def calfw_directory_mover(data, dir_name):
    co = 0
    for image in data.image_path:
        # create top directory
        if not os.path.exists(os.path.join(os.getcwd(), 'new_calfw/working/', dir_name)):
            os.makedirs(os.path.join(os.path.join(os.getcwd(), 'new_calfw/working/'), dir_name))

        data_type = data[data['image_path'] == image]['name']
        data_type = str(list(data_type)[0])
        if not os.path.exists(os.path.join(os.getcwd(), 'new_calfw/working/', dir_name, data_type)):
            os.makedirs(os.path.join(os.path.join(os.getcwd(), 'new_calfw/working/', dir_name, data_type)))
        path_from = os.path.join('images/', image)
        path_to = os.path.join('new_calfw/working/', dir_name, data_type)
