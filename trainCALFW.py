...
def run_k_fold(multi_data, X, Y, CLASSES, MODEL, BATCH_SIZE, num_folds, nomes_classes):
    ...
    sgd = optimizers.SGD(lr=1e-3, decay=1e-6, momentum=0.9, nesterov=True)
    model_vgg.compile(optimizer=sgd, loss='categorical_crossentropy', metrics=['acc'])
    model2 = model_vgg
...
def main():
    ...
    train, test = train_test_split(multi_data, test_size=0.25, random_state=42, shuffle=True,stratify=multi_data[['name']])
    print('train')
    print(train.shape)
    print(train)
