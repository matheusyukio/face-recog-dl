...
def get_model_name(name, k, batch):
    return 'model_main2' + name + '_' + str(k) + '_' + str(batch) + '.h5'
...
def run_k_fold(multi_data, X, Y, CLASSES, epoch, MODEL, BATCH_SIZE, num_folds):
    ...
    for train_index, val_index in skf.split(X, Y):
        ...
        model.load_weights(
            os.getcwd() + "/models/model_main2" + MODEL_NAME + "_" + str(fold_var) + '_' + str(BATCH_SIZE) + ".h5")
        ...
        write_results(
            get_current_time_str() + 'main2_k_fold_' + str(CLASSES) + '_' + MODEL_NAME + '_' + str(EPOCHS) + '_' + str(
                BATCH_SIZE) + '.txt', VALIDATION_ACCURACY, VALIDATION_LOSS, HISTORY)
        del history
        del model
